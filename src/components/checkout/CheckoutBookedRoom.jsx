...
const CheckoutBookedRoom = () => {
  ...
  const orderLocation =
    location.state?.orderRequestData || createInitialOrderRequest();
  const errorMsg = location.state?.error || "";

  const [orderCheckoutRequest, setOrderCheckoutRequest] =
    useState(orderLocation);
  ...
  const isloaded = orderCheckoutRequest.bookedRooms.some(room => room.amount>1);
  ...
  return (
    <>
      <div className="container mt-5">
        <section>
          <div className="row">
            <OrderSummary isLoaded={isloaded}
            />
           {orderCheckoutRequest && orderCheckoutRequest.bookedRooms?.length > 0 &&
            orderCheckoutRequest.bookedRooms.some(room => room.amount > 0) ? (
               ...
               <form className="needs-validation" onSubmit={handleSubmit}>
                 <div className="row g-3">
                   ...
                   {orderCheckoutRequest.bookedRooms?.map((room, index) => (
                     <React.Fragment key={index}>
                       ...
                       <div className="col-6">
                         <p className="total-text mt-3">
                           <strong>Total Payment: </strong>${room?.amount || ""}
                         </p>
                       </div>
                     </React.Fragment>
                   ))}
                 ...
                 <div className="form-check">
                   <label className="form-check-label">
                     This room will be ready for check-in immediately after the
                     billing process
                   </label>
                 </div>
